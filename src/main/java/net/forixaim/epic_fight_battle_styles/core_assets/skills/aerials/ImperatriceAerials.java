package net.forixaim.epic_fight_battle_styles.core_assets.skills.aerials;
...
public class ImperatriceAerials extends MidAirAttack
{
	...
	public boolean canExecute(PlayerPatch<?> executor)
	{
		if (executor.getHoldingItemCapability(InteractionHand.MAIN_HAND).getStyle(executor) == ImperatriceLumiereStyles.IMPERATRICE_SWORD)
			return super.canExecute(executor) && executor.getSkill(SkillSlots.WEAPON_INNATE).getDataManager().hasData(EFBSDataKeys.CHARGE_EXECUTING.get()) && !executor.getSkill(SkillSlots.WEAPON_INNATE).getDataManager().getDataValue(EFBSDataKeys.CHARGE_EXECUTING.get());
