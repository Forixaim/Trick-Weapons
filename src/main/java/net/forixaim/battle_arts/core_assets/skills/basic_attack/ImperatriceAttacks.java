...
public class ImperatriceAttacks extends BasicAttack
{
	...
	{
		int prevValue = container.getDataManager().getDataValue(BattleArtsDataKeys.BLAZE_COMBO.get());
		...
		container.getDataManager().setData(BattleArtsDataKeys.BLAZE_COMBO.get(), comboResetEvent.getNextValue());
	}
	...
	{
		int prevValue = container.getDataManager().getDataValue(BattleArtsDataKeys.CERCLE_DE_FEU.get());
		ComboCounterHandleEvent comboResetEvent = new ComboCounterHandleEvent(reason, playerpatch, causalAnimation, prevValue, value);
		container.getExecuter().getEventListener().triggerEvents(PlayerEventListener.EventType.COMBO_COUNTER_HANDLE_EVENT, comboResetEvent);
		container.getDataManager().setData(BattleArtsDataKeys.CERCLE_DE_FEU.get(), comboResetEvent.getNextValue());
	}
	...
	public boolean canExecute(PlayerPatch<?> executor)
	{
		if (executor.getAdvancedHoldingItemCapability(InteractionHand.MAIN_HAND).getStyle(executor) == ImperatriceLumiereStyles.IMPERATRICE_SWORD)
			return executor.getOriginal().onGround() && executor.getSkill(SkillSlots.WEAPON_INNATE).getDataManager().hasData(BattleArtsDataKeys.CHARGE_EXECUTING.get()) && !executor.getSkill(SkillSlots.WEAPON_INNATE).getDataManager().getDataValue(BattleArtsDataKeys.CHARGE_EXECUTING.get());
	...
	public void executeOnServer(ServerPlayerPatch executer, FriendlyByteBuf args)
	{
		if (executer.getHoldingItemCapability(InteractionHand.MAIN_HAND).getStyle(executer).equals(ImperatriceLumiereStyles.IMPERATRICE_SWORD))
		{
			...
			int comboCounter = dataManager.getDataValue(BattleArtsDataKeys.BLAZE_COMBO.get());
			int cercleDeFeu = dataManager.getDataValue(BattleArtsDataKeys.CERCLE_DE_FEU.get());
			...
			} else
			{
				...
				{
					//Forward Tilt
					executer.getSkill(this).getDataManager().setData(BattleArtsDataKeys.FTILT.get(), true);
				...
				{
					// Normal Attack
					executer.getSkill(this).getDataManager().setData(BattleArtsDataKeys.JAB.get(), true);
			...
			if (attackMotion != null)
			{
				...
				if (executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().hasData(BattleArtsDataKeys.HEAT.get()) && executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().getDataValue(BattleArtsDataKeys.HEAT.get()) < 50)
				{
					executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().setDataSync(BattleArtsDataKeys.HEAT.get(), executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().getDataValue(BattleArtsDataKeys.HEAT.get()) + 5, executer.getOriginal());
					if (executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().getDataValue(BattleArtsDataKeys.HEAT.get()) >= 50f)
					{
						executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().setDataSync(BattleArtsDataKeys.HEAT.get(), 50f, executer.getOriginal());
					}
				}
				LogUtils.getLogger().debug("Heat Level: {}", executer.getSkill(BattleArtsSkillSlots.BATTLE_STYLE).getDataManager().getDataValue(BattleArtsDataKeys.HEAT.get()));
			}
